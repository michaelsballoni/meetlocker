# meatlocker
Bologna Meat Locker is a file storage and retrieval application.

## Storage
The user provides a password and the file, and the server puts the file into a ZIP file protected by their password.  The server generates a key and uses this key and the password to determine where to store the ZIP file.  The key is returned to the user.  The password and key are not stored.
